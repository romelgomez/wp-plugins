...
$is_import_mode  =  ($installer_state->mode === DUPX_InstallerMode::ImportInstall);
	...
	<div class="s1-reqs" id="s1-notice-all">
		...
		<?php if (!$GLOBALS['DUPX_AC']->exportOnlyDB && ! $is_import_mode) :?>
			...
			<div class="info" id="s1-notice01">
				...
				is set to "Import Install" then	this notice can be ignored.</i>
				<br/><br/>
				<b>Options:</b>
				<ul style="margin-bottom: 0">
					...
					<li>If the mode is "Import Install" then this message can be ignored.</li>
				</ul>
			</div>
        ...
        <tr>
            ...
            <td>
                ...
                <select id="archive_engine" name="archive_engine" size="<?php echo $num_selections; ?>">
                    <?php if($installer_state->mode !== DUPX_InstallerMode::ImportInstall) : ?>
...
DUPX.processNext = function ()
{
   ...
   //moving files is only enabled in import install if is NOT only DB
   var moveCoreFiles = <?php echo ($GLOBALS['DUPX_AC']->exportOnlyDB ? 'false' : ($installer_state->mode === DUPX_InstallerMode::ImportInstall ? 'true' : 'false')); ?>;
    if(moveCoreFiles) {
