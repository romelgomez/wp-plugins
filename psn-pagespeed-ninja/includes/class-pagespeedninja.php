...
class PagespeedNinja
{
    ...
    private function loadATFCSS() {
        ...
        $tmp_filename = download_url('http://pagespeed.ninja/getcss.php?' . http_build_query($data), 60);
...
//          'clean_comment_cache', // immediately after a comment has been removed from the object cache
    ...
    private function define_admin_hooks() {
        ...
        add_action('wp_ajax_pagespeedninja_clear_cache', array($plugin_admin, 'clear_cache'));
        add_action('wp_ajax_pagespeedninja_clear_db_cache', array($plugin_admin, 'clear_db_cache'));
    ...
    private function define_public_hooks() {
        ...
        // end buffering [note: Wordpress clears all output buffers at priority=1, see default-filters.php]
        add_action('shutdown', array($plugin_public, 'shutdown'), -1000);

        add_filter('wp_cache_meta', array($plugin_public, 'wp_cache_meta'));
        add_action('wp_footer', array($plugin_public, 'wp_footer'));
    }
}
