(function () {
    ...
    jQuery(document).ready(function () {
        ...
        jQuery('#clear_cache').on('click', function () {
            ...
            jQuery.post(ajaxurl, {action: 'pagespeedninja_clear_cache'}, function () {
                $el.removeAttr('disabled');
            });
        });

        jQuery('#clear_db_cache').on('click', function () {
            var $el = jQuery(this);
            $el.attr('disabled', 'disabled');
            jQuery.post(ajaxurl, {action: 'pagespeedninja_clear_db_cache'}, function () {
                $el.removeAttr('disabled');
            });
        });
    ...
    function ATFSetText(id, content) {
    ...
    function ATFExternal(id, url) {
        jQuery.ajax({
            url: 'http://pagespeed.ninja/getcss.php',
            data: {
                url: url
            },
            success: function (content) {
                ATFSetText(id, content);
            },
            cache: false
        });
    }

    function ATFInternal(id, url) {
        getATFCSS(url, function (content) {
            ATFSetText(id, content);
        });
    }

    function autoGenerateATF(id, local) {
