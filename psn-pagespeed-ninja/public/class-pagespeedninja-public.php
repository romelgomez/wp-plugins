...
class PagespeedNinja_Public
{
    ...
    public function shutdown()
    {
        ...
        if (ob_get_level() === 0) {
            trigger_error('PageSpeed Ninja: no output buffer');
            return;
        }
        ...
        if ($buffer !== '') {
            ...
            // @todo store ready-to-use regex in the database, and extract filenames in backend form only
            $css_excludemergeregex = null;
            if (!empty($options['css_excludelist'])) {
                $list = explode("\n", trim($options['css_excludelist']));
                ...
                if (count($list)) {
                    $css_excludemergeregex = '#(?:' . implode('|', $list) . ')#';
                }
            }

            $js_excludemergeregex = null;
            if (!empty($options['js_excludelist'])) {
                $list = explode("\n", trim($options['js_excludelist']));
                foreach ($list as $i => $line) {
                    $line = trim($line);
                    if ($line === '') {
                        unset($list[$i]);
                ...
                if (count($list)) {
                    $js_excludemergeregex = '#(?:' . implode('|', $list) . ')#';
                }
            }
            ...
            if ($options['psi_LeverageBrowserCaching']) {
                if ($options['loadexternal']) {
                    $ress_options['plugins']['Ressio_Plugin_UrlLoader'] = null;
                }
            }

            if ($options['psi_MainResourceServerResponseTime']) {
                // @todo MainResourceServerResponseTime (e.g. static caching)
            ...
            if ($options['psi_MinifyCss']) {
                ...
                $ress_options['css']['loadurl'] = (bool)$options['css_loadurl'];
            ...
            if ($options['psi_MinifyJavaScript']) {
                ...
                $ress_options['js']['loadurl'] = (bool)$options['js_loadurl'];
            ...
            if ($options['psi_MinimizeRenderBlockingResources']) {
                // @todo separate above-the-fold and non-blocking-js settings
            ...
            if ($options['psi_MinifyJavaScript']) {
                $ressio->di->dispatcher->addListener('HtmlIterateTagSCRIPTBefore', array($this, 'collectScriptURLs'));
            }
            if ($options['psi_MinifyCss']) {
                $ressio->di->dispatcher->addListener('HtmlIterateTagLINKBefore', array($this, 'collectStyleURLs'));
            }
            // @todo Usability features
    ...
    public function RessioAddViewportTag($event, $optimizer)
    {
        ...
        $optimizer->prependHead(
            array('meta', array('name' => 'viewport', 'content' => 'width=' . $width), null)
        );
    }
    ...
    public function wp_footer()
    {
        ...
        if ($options['footer'] === '1') {
            echo '<small class="pagespeedninja" style="display:block;text-align:center">'
                . sprintf(__('Optimized with <a href="%s">PageSpeed Ninja</a>'), 'http://pagespeed.ninja/')
                . '</small>';
        } else {
            echo '<!-- '
                . sprintf(__('Optimized with <a href="%s">PageSpeed Ninja</a>'), 'http://pagespeed.ninja/')
