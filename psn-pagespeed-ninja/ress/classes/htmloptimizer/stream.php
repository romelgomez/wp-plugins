                    ...
                    if (isset($attributes['ress-nomerge'])) {
                        ...
                        if ($merge) {
                            ...
                            if ($regex !== null && preg_match($regex, $src)) {
                                ...
                                $merge = ($srcFile !== null) && ($ext === 'css') && $this->di->filesystem->isFile($srcFile);
                            }
                        }
                    }
...
//                      $media = $this->filterMedia($media);
                ...
                case 'script':
                    ...
                    if (!isset($attributes['src'])) { // inline
                        ...
                        $node->content = preg_replace(array('#^\s*<!--.*?[\r\n]+#', '#//\s*<!--.*$#m', '#//\s*-->.*$#m', '#\s*-->\s*$#'), '', $node->content);
                        // @todo remove CDATA wrapping
                        $autoasync = ($autoasync && (strpos($node->content, '.write') === false || !preg_match('#\.write(?!\(\))#', $node->content)));
                        ...
                        if (isset($attributes['ress-nomerge'])) {
                            ...
                            if ($merge && isset($node->attributes['id'])) {
                                ...
                                if (preg_match('/([\'"])#?' . preg_quote($id, '/') . '\1', $node->content)) {
                                    $merge = false;
                                }
                            }
                        }
                        ...
                        if ($merge) {
                            ...
                            if ($regex !== null && preg_match($regex, $src)) {
                                ...
                                $srcFile = $this->urlRewriter->urlToFilepath($src);
                                $ext = pathinfo($srcFile, PATHINFO_EXTENSION);
                                $merge = ($srcFile !== null) && ($ext === 'js') && $this->di->filesystem->isFile($srcFile);
                            }
                        }
    ...
    public function tagToString($tagName, $attributes, $content)
    {
        ...
        $out .= ($content === null) ? '>' : '>' . $content . '</' . $tagName . '>';
        return $out;
    }
    ...
    public function prependHead($nodedata)
    {
        ...
        foreach ($this->dom as &$node) {
            if (is_string($node) && strpos($node, '<head') === 0) {
