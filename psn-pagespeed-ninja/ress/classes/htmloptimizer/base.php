...
abstract class Ressio_HtmlOptimizer_Base implements IRessio_HtmlOptimizer
{
    ...
    public function cssMinifyInline($str, $srcBase = null, $targetBase = null)
    {
        try {
            ...
            error_log('Catched error in Ressio_HtmlOptimizer_Base::cssMinifyInline: ' . $e->getMessage());
            return $str;
        }
    }
    ...
    public function jsMinifyInline($str)
    {
        try {
            ...
            error_log('Catched error in Ressio_HtmlOptimizer_Base::jsMinifyInline: ' . $e->getMessage());
            return $str;
        }
    }
    ...
    public function allowedCDN($src)
    {
        if (strpos($src, '//') === false) {
            return false;
        }
        $host = parse_url($src, PHP_URL_HOST);
        return in_array($host, $this->config->allowedcdn, true);
    }
}
