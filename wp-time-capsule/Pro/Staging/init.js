jQuery(document).ready(function($) {
	...
	jQuery('body').on('click', '#ask_copy_staging_wptc', function (e){
		...
		swal({
			title: '<div class="wptc-swal-title">Copy live to staging?</div>',
			html: '<div class="wptc-swal-msg" >Clicking on Yes will continue to copy your live site to staging site. Are you sure want to continue ?</div>',
			type: 'warning',
		...
		try{
			...
			if(data.status === 'continue'){
				...
				if (data.is_restore_to_staging) {
					startRestore(null, null, null, null);
				}
			} else {
				console.log('Something went wrong !');
				// jQuery("#staging_progress_bar_note").html('Error: Something went wrong, try again.');
			}
		} catch(err){
			console.log(err);
		}
...
function wptc_choose_update_in_stage(update_items, type){
	swal({
		title: '<div class="wptc-swal-title">Choose option !</div>',
		html: "<div class='wptc-swal-msg'>Want to try this update in staging site?</div>",
		type: 'warning',
		showCancelButton: true,
		...
		confirmButtonText: 'Update in staging',
			...
			swal({
				...
				html: "<div class='wptc-swal-msg'>This will just perform the update in the staging site, Other changes are safe!</div>",
				type: 'warning',
				showCancelButton: true,
				showConfirmButton: true,
				confirmButtonColor: '#3085d6',
				cancelButtonColor: '#d33',
				confirmButtonText: 'Yes, Update in staging',
				cancelButtonText: 'No, Cancel',
			}).then(function () {
			...
			swal({
				title: '<div class="wptc-swal-title">Are you sure?</div>',
				html: "<div class='wptc-swal-msg'>This will erase your entire staging site and do fresh staging then initiate the update!</div>",
				type: 'warning',
				showCancelButton: true,
				showConfirmButton: true,
				confirmButtonColor: '#3085d6',
				cancelButtonColor: '#d33',
				confirmButtonText: 'Yes, Stage and update',
				cancelButtonText: 'No, Cancel',
			}).then(function () {
...
function wptc_save_upgrade_meta_in_staging(update_items, type, choice){
	...
	jQuery.post(ajaxurl, {
		...
		if (choice === 'update_in_staging') {
			swal(
				'<div class="wptc-swal-title">Update initiated!</div>',
				'<div class="wptc-swal-msg">Continue your work, we will update you once update is done on staging site!</div>',
				'success'
			);
			...
			swal(
				'<div class="wptc-swal-title">Update in staging initiated !</div>',
				'<div class="wptc-swal-msg">Continue your work, we will update you once state and update is done!</div>',
				'success'
			);
