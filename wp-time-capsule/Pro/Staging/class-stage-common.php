...
class WPTC_Stage_Common{
	...
	public function clone_table_structure($table, $new_table){
		...
		if ($is_cloned === false) {
			$this->log('Creating table ' . $table . ' has been failed', 'staging_progress', $this->staging_id);
			wptc_log('Creating table ' . $this->wpdb->last_error . ' has been failed.', '--------Failed-------------');
			return false;
		}

		$this->log("Created table " . $table, 'staging_progress', $this->staging_id);
		return true;
	}
	...
	public function clone_table_content($table, $new_table, $limit, $offset){
		while(true){
			...
			if ($inserted_rows !== false) {
				if ($offset != 0) {
					$this->log('Copy database table: ' . $table . ' DB rows: ' . $offset , 'staging_progress', $this->staging_id); //create staging id
				}
				...
				if(is_wptc_timeout_cut()){
					...
					die_with_json_encode(array('status' => 'continue', 'msg' => 'Cloning ' . $table . '(' . $offset . ')' , 'percentage' => 20));
				}
			} else {
				$this->processed_db->update_iterator($table, -1); //Done
				break;
				$this->log('Error: '.$this->wpdb->error.'inserting rows failed! Rows will be skipped. Offset: ' . $offset, 'staging_progress', $this->staging_id);
