...
class Wptc_Staging extends WPTC_Privileges {
	...
	private function init_seek_iterator(){
		...
		$this->seek_file_iterator = new WPTC_Seek_Iterator($this, $type = 'LIVE_TO_STAGING', $timeout_limit);
	}
	...
	public function choose_action($path = false, $reqeust_type = false){
		...
		if ($status === 'stop_staging') {
			...
			die_with_json_encode(array('status' => 'success', 'msg' => 'Stopping Staging process.', 'percentage' => 0));
		}
		...
		if ($status === 'test_bridge_over' || $status === 'db_clone_progress') {
			...
			die_with_json_encode(array('status' => 'continue', 'msg' => 'Database is cloned.', 'percentage' => 30));
		}
		...
		if($status === 'db_clone_over' || $status === 'copy_files_progress'){
			...
			die_with_json_encode(array('status' => 'continue', 'msg' => 'Files are copied successfully' , 'percentage' => 75));
		}

		if($status === 'copy_files_over'){
			wptc_manual_debug('', 'start_replace_links_staging');
			$this->process_replace_links_req($status);
			die_with_json_encode(array('status' => 'continue', 'msg' => 'Links are replaced.', 'percentage' => 90));
		}

		if($status === 'replace_links_over'){
			wptc_manual_debug('', 'start_completing_staging');
			$this->process_staging_completed_req();
			die_with_json_encode(array('status' => 'continue', 'msg' => 'Resetting flags...', 'percentage' => 95));
		}
		...
		if ($status === 'staging_completed') {
			...
			die_with_json_encode($response);
		}

		die_with_json_encode(array('status' => 'error', 'msg' => 'Cannot fetch the current status. Please try again', 'percentage' => 100));
	}
	...
	private function same_server_copy_bridge_files($path = false, $do_not_die = false){
		...
		if ($this->filesystem->exists($this->same_staging_bridge_dir) !== true && $do_not_die === false) {
			...
			$this->logger->log("Testing connection by uploading bridge file is failed", 'staging_progress', $this->staging_id);
			die_with_json_encode(array('status' => 'error' , 'msg' => 'Cannot create staging folder.'));
		}
		...
		if(!$cp_result && $do_not_die === false){
			...
			$this->logger->log("Testing connection by uploading bridge file is failed", 'staging_progress', $this->staging_id);
			die_with_json_encode(array('status' => 'error' , 'msg' => 'Cannot create copy bridge file.'));
		}
		...
		if (!$this->is_force_update && $do_not_die === false) {
			...
			die_with_json_encode(array('status' => 'continue', 'msg' => 'Test bridge copied.', 'percentage' => 10));
		}
	}
	...
	private function copy_selected_folders(){
		...
		while(!$break){
			...
			$file = wptc_add_abspath($dir_meta->name, false);
	...
	private function copy_file($live_file, $update_status = false){
		...
		if (!$copy_status) {
			$this->logger->log('Could not copy this file - '.$live_file, 'staging_progress', $this->staging_id);
		}
	...
	private function copy_dir($live_path, $offset, $deep_dirs){
		...
		try{
			...
			if (wptc_is_file_iterator_allowed_exception($exception_msg)) {
				$this->logger->log($exception_msg . ' Please exclude this folder to make staging work.', 'staging_progress', $this->staging_id);
				die_with_json_encode(array('status' => 'error' , 'msg' => $exception_msg . ' Please exclude this folder to make staging work.'));
			}

			if (!wptc_is_seeking_exception($exception_msg)) {
				die_with_json_encode(array('status' => 'error' , 'msg' => $exception_msg));
			}
	...
	private function check_timeout_iter_file($path, $offset, &$temp_counter, $timeout_limit){
		...
		die_with_json_encode(array('status' => 'continue', 'msg' => $this->app_functions->get_processing_files_count($type = 'internal_staging'), 'percentage' => 50));
	}
	...
	public function is_staging_need_request(){
		...
		if(!$this->is_any_staging_process_going_on()){
			die_with_json_encode(array('status' => false, 'check_again' => false));
		}
		...
		if ($last_staging_ping + ( WPTC_TIMEOUT * 2 )  <  time() )  {
			die_with_json_encode(array('status' => true, 'check_again' => false));
		}

		die_with_json_encode(array('status' => false, 'check_again' => true));
	}
	...
	private function same_server_clone_db() {
		...
		foreach ($wp_tables as $table) {
			...
			if (!$this->is_same_server_running()) {
				die_with_json_encode(array('status' => 'success', 'msg' => 'Staging process stopped.', 'percentage' => 0));
			}
	...
	public function same_server_replace_links() {
		...
		$this->logger->log('Updating db prefix "' . $this->wpdb->base_prefix . '" to  "' . $new_prefix . '" has been done succesfully.', 'staging_progress', $this->staging_id);
	...
	private function disable_search_engine_indexing($new_prefix){
		...
		$this->replace_links_obj->discourage_search_engine($new_prefix, $reset_permalink = true);
	...
	private function replace_old_url(){
		...
		$this->replace_links_obj->replace_uri($old_url, $new_url, $old_file_path, $new_file_path, $table_prefix, $tables);
	}
	...
	private function create_default_htaccess(){
		...
		$this->logger->log('.htaccess has been modified.', 'staging_progress', $this->staging_id);
	...
	private function same_server_set_staging_path($path = false){
		...
		// if (wptc_is_abspath($this->same_staging_folder)) {
		// 	if($this->config->get_option('same_server_staging_status')){
		// 		die_with_json_encode(array('status' => 'error' , 'msg' => 'Staging folder is empty, Please stop staging and re-initiate the staging.'));
		// 	}
		// }
	}
	...
	public function staging_to_live_copy_files(){
		...
		if (WPTC_WP_CONTENT_DIR === $stage_wp_content) {
			wptc_log(array(), '--------Dont copy to live site--------');
			return false;
		}
	...
	public function staging_to_live_copy_db(){
		...
		if (apply_filters('is_restore_to_staging_wptc', '')) {
				$tables = array(
					...
					$this->wpdb->prefix . 'wptc_auto_backup_record'       => 'schema',
					$this->wpdb->prefix . 'wptc_current_process'          => 'schema',
					$this->wpdb->prefix . 'wptc_debug_log'                => 'schema',
					...
					$this->wpdb->prefix . 'wptc_backup_names'             => 'full',
					$this->wpdb->prefix . 'wptc_excluded_files'           => 'full',
					$this->wpdb->prefix . 'wptc_excluded_tables'          => 'full',
					$this->wpdb->prefix . 'wptc_included_files'           => 'full',
					$this->wpdb->prefix . 'wptc_included_tables'          => 'full',
	...
	private function things_to_do_after_completion(){
		...
		$this->update_settings_on_staging_site('R2S_deep_links_completed', false);
	...
	private function update_settings_on_staging_site($name, $value){
		...
		if ($old_value === false) {
			return $this->wpdb->insert( $staging_prefix . 'wptc_options',	array(
					'name' => $name,
					'value' => $value,
					)
				);
		...
		return	$this->wpdb->update(	$staging_prefix . 'wptc_options',
				array('value' => $value),
				array('name' => $name)
			);
	}
