...
class WPTC_Stage_To_Live{
	...
	private function clone_db(){
		...
		foreach ($tables as $table) {
			...
			$new_table = str_replace($this->wpdb->base_prefix, $live_prefix, $table);
	...
	private function get_all_tables(){
		...
		foreach ($wp_tables as $table) {
			if(strpos($table, $this->wpdb->base_prefix) !== false){
				$site_tables[] = $table;
			}
		}
	...
	private function replace_site_links(){
		...
		if ($result_options === false || $result_usermeta === false) {
			// $this->logger->log("Updating table $new_prefix has been failed.". $this->wpdb->last_error, 'staging_progress', $this->staging_id);
	...
	private function get_hash_by_folders(){
		...
		$this->app_functions->die_with_json_encode(array('status' => 'continue', 'msg' => 'Coping files...', 'percentage' => 50));
	}
	...
	private function get_hash_dir($path, $offset, &$temp_counter, $deep_dirs){
		...
		$seek_file_iterator = new WPTC_Seek_Iterator($this, $type = "STAGING_TO_LIVE", 500);
	...
	private function is_file_modified($stage_file){
		...
		$live_file = str_replace($this->stage_path, $this->live_path, $stage_file);
	...
	private function is_wptc_dir($file){
		if(strpos($file, WPTC_PLUGIN_DIR) !== false){
			return true;
		}
	...
	private function final_things_to_do(){
		...
		$this->completed();
		wptc_manual_debug('', 'start_completed_S2L');
	...
	public function check_permissions() {
		...
		wp_die( sprintf ( __('Access denied. You need to <a href="%1$s" target="_blank">Login</a> first','wptc'), get_admin_url() ) );
	}
	...
	private function should_disable_login(){
		return (
			...
			$this->config->get_option('internal_staging_enable_admin_login') &&
			!$this->is_wptc_request() &&
			(!current_user_can("administrator") && !$this->is_login_page() && !is_admin())
		);
	}
