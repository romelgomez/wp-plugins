...
jQuery(document).ready(function($) {
			...
			preConfirm: function (description) {
				return new Promise(function (resolve, reject) {
					if (description) {
						...
						swal.hideLoading();
						swal.showValidationError('Description cannot be empty!');
					}
				})
			},
	...
	jQuery("#wptc_clear_log").on('click', function() {
		...
		swal({
			...
			html: '<div class="wptc-swal-msg" >Are you sure you want to permanently delete these logs?</div>',
			type: 'warning',
	...
	jQuery('body').on('click', '#wptc_make_this_original_site', function() {
		...
		swal({
			title: '<div class="wptc-swal-title">Are you sure?</div>',
			html: '<div class="wptc-swal-msg"> The site ' + old_url + ' will be replaced to ' + new_url + ' in WP Time Capsule and don\'t worry all your exisiting backups are safe.</div>',
			type: 'warning',
			showCancelButton: true,
			confirmButtonColor: '#3085d6',
			cancelButtonColor: '#d33',
			...
			swal(
				'<div class="wptc-swal-title">Replaced! </div>',
				'<div class="wptc-swal-msg">Site URL has been replaced successfully!</div>',
				'success'
				);
	...
	jQuery('body').on('click', '#wptc_make_this_fresh_site', function() {
		...
		swal({
			title: '<div class="wptc-swal-title">Are you sure?</div>',
			html: '<div class="wptc-swal-msg"> All the backups will be erased on this site and you need to go through the initial setup once again on this site, meanwhile <strong>' + old_url + '</strong> will be working just as fine. </div>',
			type: 'warning',
			showCancelButton: true,
			confirmButtonColor: '#3085d6',
			cancelButtonColor: '#d33',
			...
			swal(
				'<div class="wptc-swal-title">This site is ready for backups!</div>',
				'<div class="wptc-swal-msg"> All backups are reset. Please go through the initial setup process to enable backup on this site.</div>',
				'success'
				);
...
function disable_settings_wptc(){
	if (jQuery("#start_backup_from_settings").text() != 'Stopping backup...') {
		...
		jQuery("#select_wptc_backup_slots, #select_wptc_default_schedule, #wptc_timezone").addClass('disabled').attr('disabled', 'disabled');
		jQuery('.change_dbox_user_tc').addClass('wptc-link-disabled');
...
function enable_settings_wptc(){
	if (jQuery("#start_backup_from_settings").text() != 'Starting backup...') {
		...
		jQuery("#select_wptc_backup_slots, #select_wptc_default_schedule, #wptc_timezone").removeClass('disabled').removeAttr('disabled');
...
function ask_backup_name_wptc(type){
	...
	if (type === 'bbu') {
		...
		return show_notification_bar(data);
	}
		...
		preConfirm: function (name) {
			return new Promise(function (resolve, reject) {
				if (name) {
					...
					swal.hideLoading();
					swal.showValidationError('Backup name cannot be empty!');
				}
			})
		},
		allowOutsideClick: true
	...
	jQuery.post(ajaxurl, {
		...
		if (data.success) {
			swal(
				...
				'<div class="wptc-swal-msg">Issue sent successfully, We will get in touch with you shortly.</div>',
				'success'
			);
		} else {
			swal(
				'<div class="wptc-swal-title">Failed</div>',
				'<div class="wptc-swal-msg">Issue sending failed, try after sometime or email us at <a href="mailto:help@wptimecapsule.com?Subject=Contact" target="_top">help@wptimecapsule.com</a>!</div>',
				'error'
			);
		}
	});
...
function yes_delete_logs_wptc() {
	jQuery.post(ajaxurl, {
		...
		if (data.success) {
			swal(
				'<div class="wptc-swal-title">Success</div>',
				'<div class="wptc-swal-msg">Logs are deleted!</div>',
				'success'
			);
			...
			swal(
				'<div class="wptc-swal-title">Failed</div>',
				'<div class="wptc-swal-msg">Failed to remove logs from Database!</div>',
				'error'
			);
		}
	});
...
function process_backup_status(backup_progress, prog_percent) {
	if (backup_progress == '') {
		return update_backup_status('backup_completed');
	}

	if (backup_progress.meta.running) {
		return update_backup_status('meta', backup_progress.meta.message);
	}

	if(backup_progress.db.running){
		return update_backup_status('db', backup_progress.db.progress);
	}

	if(backup_progress.files.processing.running){
		return update_backup_status('analyze', backup_progress.files.processing.progress);
	}

	if(backup_progress.files.processed.running) {
		return update_backup_status('files', prog_percent);
	}

}

function update_backup_status(type, value) {
...
function load_cron_status_failed_popup(status_code, err_msg, cron_url, ips){
	...
	swal({
		title: '<div class="wptc-swal-title">Connection failed !</div>',
		html: '<div class="wptc-swal-msg" style ="max-width: 500px; max-height: 400px; overflow: auto;"> ' + err_msg + '  </div> <br><br><br> <div style="font-size: 13px;text-align: justify;"> Note: Please whitelist the IP Address ('+ips+') to access the URL : <a style="text-decoration: none;" href="'+cron_url+'">'+cron_url+'</a> or get in touch with your hosting provider to have it whitelisted. You can check the Cron Status anytime under WPTC -> Settings or email us at <a href="mailto:help@wptimecapsule.com?Subject=Contact" target="_top">help@wptimecapsule.com</a></div></div>',
		type: 'error',
	});
...
function load_custom_popup_wptc(show, type, show_all_page, title, msg, footer){
	...
	swal({
		title: '<div class="wptc-swal-title" style="font-size: 19px !important;"> '+ title + ' </div>',
		html: '<div class="wptc-swal-msg">'+ msg +'</div> <br><br> <div style="font-size: 14px;">' + footer + '</div>',
	});
...
function save_manual_backup_name_wptc(name){
	jQuery.post(ajaxurl, {
		...
		if(obj.status && obj.status == 'success'){
			swal(
				'<div class="wptc-swal-title">Success </div>',
				'<div class="wptc-swal-msg">Backup name saved :)</div>',
				'success'
			);
			setTimeout(function(){
...
function wptc_request_failed(){
	swal(
		'<div class="wptc-swal-title">Request failed!</div>',
		'<div class="wptc-swal-msg">Please try again !</div>',
		'error'
	);
}