...
class Wptc_Plans
{
	...
	private function get_formatted_price_div($single_plan_info)
	{
		...
		if($single_plan_info['plan_interval'] == 'yearly'){
			$single_plan_info['plan_amt'] = ($single_plan_info['plan_amt'] / 12);
		...
		$monthly_or_lifetime = ($single_plan_info['plan_interval'] == 'lifetime') ? 'lifetime' : 'monthly';
		...
		$price_text .= '<div class="dollar_group">
			...
			<span class="dollar_wptc"><span class="kutty_dollar" style="'.$dont_show_dollar.'">$</span>' . $single_plan_info['plan_amt'] . '</span> <span class="price_interval_wptc" style="'.$dont_show_dollar.'"> /'.$monthly_or_lifetime.'</span></div>';
	...
	private function generate_sub_div($single_plan_info = null, $plan_id = null, $is_card_added = false, $encoded_email = false, $encoded_pwd = false)
	{
		...
		$sub_div .= '<div class="package_wptc '. $plan_disable_class.' '.$single_plan_info['plan_group'].' '.$single_plan_info['plan_interval'].'" plan_group_class="'.$single_plan_info['plan_group'].'" style="'.$hide_yearly_style. ' ' . $plan_legacy_hide .'">
		  <div class="package_wptc_border">
		  	<div class="plan_interval_group" style="'.$hide_radio_buttons.'">
		  		<label class="radio-inline plan_interval_change_wptc"><input type="radio" '.$monthly_radio_checked.' name="plan_int_radio_'.$plan_id.'">monthly</label>
    ...
    public function echo_plan_box_div_wptc()
    {
        ...
        wptc_log($plan_info, "--------plan_info-----echo_plan_box_div_wptc---");
		...
		<div class="plans-angle-header-wptc">  
          <div class="top-angle-wptc"></div>
          <div class="bottom-angle-wptc"></div>
        </div>
