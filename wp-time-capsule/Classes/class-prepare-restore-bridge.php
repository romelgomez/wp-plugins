...
class WPTC_Prepare_Restore_Bridge{
	...
	private function pre_check(){
		if (!$this->app_functions->server_has_free_space()) {
			die_with_json_encode(array('error' => 'There is not enough free space on you server to restore.'));
		}
	}
	...
	private function set_restore_request($request){
		if ( empty($request) ) {
			die_with_json_encode(array('error' => 'Request is empty.'));
		}
	...
	public function prepare(){
		try {
			...
			if (!empty($copy_result) && !empty($copy_result['error'])) {
				...
				die_with_json_encode($copy_result);
			}
				...
				array(
					'restoreInitiatedResult' =>
						array(
							...
							'is_restore_to_staging' => $is_restore_to_staging,
			...
			$this->send_restore_report('FAILED');
			die_with_json_encode(array('error' => $e->getMessage()));
		}
	}
	...
	public function set_fs(){
		...
		if (!$wp_filesystem) {
			...
			if (!$wp_filesystem) {
				die_with_json_encode(array('error' => 'Cannot initiate WordPress file system.'));
			}
		}
	...
	public function copy_bridge_files() {
		...
		$restore_id = $this->config->get_option('restore_action_id');
		...
		if (!$copy_res) {
			$this->logger->log('Failed to Copy Bridge files', 'restore_error', $restore_id);
		...
		foreach ($plugin_folders_to_copy as $v) {
			...
			if (!$this->fs->is_dir($bridge_dir_sub)) {
				if (!$this->fs->mkdir($bridge_dir_sub, FS_CHMOD_DIR)) {
					$this->logger->log('Failed to create bridge directory while restoring . Check your folder permissions', 'restore_error', $restore_id);
		...
		$this->logger->log('Bridge Files are prepared successfully', 'restore_error', $restore_id);
		return true;
	}
	...
	private function create_config_file(){
		...
		$this->logger->log('Error Creating config like file.', 'restore_error', $this->restore_id);
	...
	public function create_config_like_file() {
		...
		if (!apply_filters('is_restore_to_staging_wptc', '')) {
			...
			$plugin_dir           = $content_dir . '/plugins/' . WPTC_STAGING_PLUGIN_DIR_NAME;
		...
		define('WPTC_UPLOADS_DIR', '" . $uploads_dir . "');

		if(!defined('WPTC_RELATIVE_UPLOADS_DIR'))
		define('WPTC_RELATIVE_UPLOADS_DIR', '" . WPTC_RELATIVE_UPLOADS_DIR . "');
		...
		define('WP_CONTENT_DIR', ABSPATH . '".basename( $content_dir )."');

		if ( !defined('WP_LANG_DIR') )
		define('WP_LANG_DIR', ABSPATH . '".basename( $content_dir ) . "/lang');
