...
class WPTC_BackupController {
	...
	public function __construct($output = null) {
		...
		$this->tcapsule_path = 	wptc_get_tmp_dir() . '/tCapsule';
	...
	public function backup_path($path, $always_include_file = null, $backup_id = null) {
		...
		if (!$this->config->get_option('gotfileslist', true)) {
			...
			$this->logger->log(__("Starting File List Iterator.", 'wptc'), 'backup_progress', $backup_id);
			...
			$this->logger->log(__("Ending File List Iterator.", 'wptc'), 'backup_progress', $backup_id);
		...
		while ($is_queue) {
			...
			foreach ($Qfiles as $file_info) {
				...
				if (!is_readable($file)) {
					...
					if (strpos($file, 'tCapsule/backups') !== false) {
						continue;
					}
				...
				if ($cloud_response == 'exist') {
					...
					wptc_log($cloud_response, '--------$cloud_response--------');
					$this->logger->log($cloud_response['error'], 'backup_error', $backup_id);
					wptc_log(array(), '---------GOT ERROR ON THIS------------');
					continue;
				}
	...
	public function execute($type = '') {
		...
		if ($this->config->get_option('bbu_upgrade_process_running')) {
			$this->config->complete(null, false);
			return false;
		}
		...
		try {
			if ((defined('DEFAULT_REPO') && DEFAULT_REPO) && !$this->dropbox->is_authorized()) {
				$this->logger->log(__('Your ' . DEFAULT_REPO . ' account is not authorized yet.', 'wptc'), 'backup_error', $backup_id);
			...
			$result = $this->backup_path(get_tcsanitized_home_path(), $this->db_backup->get_file(), $backup_id);
			...
			$msg = $e->getMessage();
			$this->logger->log(__('A error occured on backup ', 'wptc') . $msg, 'backup_error', $backup_id);
	...
	private function backup_database($backup_id){
		...
		if ($status === 'failed') {
			...
			if ($dbStatus == WPTC_DatabaseBackup::IN_PROGRESS) {
				$this->logger->log(__('Resuming SQL backup.', 'wptc'), 'backup_progress', $backup_id);
			} else {
				$this->logger->log(__('Starting SQL backup.', 'wptc'), 'backup_progress', $backup_id);
			}

			$this->db_backup->execute();
			$this->logger->log(__('SQL backup complete. Starting file backup.', 'wptc'), 'backup_progress', $backup_id);
	...
	private function add_completed_log($backup_id){
		...
		$this->logger->log(__('Backup complete.', 'wptc'), 'backup_complete', $backup_id);
		$this->logger->log(sprintf(__('Total time taken to complete the full backup process is %d secs.', 'wptc'), $backup_all_time_taken), 'backup_complete', $backup_id);
		$this->logger->log(sprintf(__('A total of %s files were processed.', 'wptc'), $this->processed_file_count), 'backup_complete', $backup_id);
	...
	public function  proper_backup_force_complete_exit($msg = false){
		...
		$this->logger->log(__('Backup stopped', 'wptc'), 'backup_error', $backup_id);
		$this->db_backup->clean_up();
	...
	public function backup_now($type) {
		...
		do_action('send_report_data_wptc', getTcCookie('backupID'), 'BACKUP', 'STARTED');
	...
	public function stop($process = null) {
		...
		if ($process == 'restore' || $process == 'logout' ) {
			...
			$this->logger->log(__('You stopped this backup manually from settings', 'wptc'), 'backup_error',$cur_backup_id);
	...
	public function unlink_current_acc_and_backups() {
		...
		$this->logger->log('Current account unlinked and backups data were deleted', 'unlink_current_acc_and_backups');
	...
	public function clear_prev_repo_backup_files_record($reset_inc_exc = false) {
		$this->logger->log('Current Repo Backup Files were Removed', 'clear_prev_repo_backup_files_record');
	...
	private function get_hash_dir($path, $offset, &$temp_counter, $deep_dirs){
		...
		try{
			...
			if (wptc_is_file_iterator_allowed_exception($exception_msg)) {
				...
				$this->logger->log(__($exception_msg . ' Please exclude this folder from the backup to continue the backup.', 'wptc'), 'backup_error', getTcCookie('backupID') );
				$this->proper_backup_force_complete_exit('file_iterator_allowed_exception :' . $exception_msg . ' So thrown error and asked them to exclude');
			}
	...
	public function process_file($iterator, $path, &$counter, $iterator_loop_limit, &$query, $key){
		...
		$relative_file = wptc_remove_abspath( $file , false);
		...
		if(!$this->processed_files->is_file_modified_from_before_backup($relative_file, $size, $file_hash)){
			// wptc_log(array(), '--------SKIP 2--------');
			return $this->app_functions->check_timeout_iter_file($path, $counter, $iterator_loop_limit, $query, $key);
		}

		wptc_log($relative_file, '---------------$relative_file-----------------');
	...
	public function is_skip($file, $is_dir = false){
		...
		// if ($this->file_list->in_ignore_list($file) && !$this->exclude_class_obj->is_included_file($file) && $always_backup === false ) {
		// 	return true;
		// }

		//skip auto backup files if its not an auto backup
		if ($this->is_auto_backup) {
			return false;
		}
	...
	private function add_backup_general_data($backup_id){
		...
		$this->wpdb->insert($this->wpdb->base_prefix . 'wptc_backups',
					array(
	...
	private function create_current_state_file_if_not_exist(){
		...
		@file_put_contents($file , wptc_remove_abspath($backup_dir) . "/wptc_dummy_file.txt\n", FILE_APPEND);
