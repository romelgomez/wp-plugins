...
class WPTC_S3Facade {
	...
	public function enable_versioning() {
		$result = $this->client->putBucketVersioning(array(
	...
	public function is_authorized($is_restore = false) {
		...
		try {
			$userInfoService = $this->client->headBucket(array(
				'Bucket' => $this->as3_bucket,
			));
		} catch (Exception $e) {
			wptc_log($type, "--------e s3 exection--------");
			return $this->process_exception($e);
		}
		return true;
	}

	public function validate_max_revision_limit() {
		try{
			$result = $this->client->getBucketLifecycleConfiguration(array(
				'Bucket' => $this->as3_bucket,
			));
		} catch(Exception $e){
		...
		if($wptc_rule['Status'] !== 'Enabled' ){
			return WPTC_DEFAULT_MAX_REVISION_LIMIT;
		}

		if($wptc_rule['NoncurrentVersionExpiration']['NoncurrentDays'] >= 120  ){
			return 120;
		}
	}
	...
	public function abort_all_multipart_uploads($parent_folder) {
		...
		foreach ($running_multi_part_uploads['Uploads'] as $k => $v) {
			...
			$safe_to_abort = (strpos($this_key, $this_site_path) === 0) ? true : false;
	...
	private function process_exception($e){
		...
		if ($this->is_forbidden_error($http_code, $err_msg)) {
			WPTC_Factory::get('logger')->log('Amazon S3 authorization is revoked - HTTP (' . $http_code . ') ' . $err_msg . ' ', 'connection_error');
