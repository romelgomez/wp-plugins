...
class Wptc_Settings extends Wptc_Settings_Init {
	...
	private function include_header_files(){
		...
		// wp_enqueue_style('wptc-sweetalert-css', plugins_url() . '/' . WPTC_TC_PLUGIN_NAME . '/lib/sweetalert.css', array(), WPTC_VERSION);
		wp_enqueue_script('wptc-settings-common-js', plugins_url() . '/' . WPTC_TC_PLUGIN_NAME . '/js/settings-common.js', array(), WPTC_VERSION);
		// wp_enqueue_script('wptc-sweetalert-js', plugins_url() . '/' . WPTC_TC_PLUGIN_NAME . '/lib/sweetalert.min.js', array(), WPTC_VERSION);
	}
	...
	private function init_settings_tabs(){
		...
		$this->tabs['net'] = __( 'Network', 'wp-time-capsule' );
	...
	public function get_user_excluded_extenstions(){
		return $this->config->get_option('user_excluded_extenstions');
	}
	...
	public function get_revision_limit_html(){
		...
		switch ($eligible_revision_limit) {
			case ($eligible_revision_limit > WPTC_DEFAULT_MAX_REVISION_LIMIT):
				$html .= $this->get_dropdown_option_html($eligible_revision_limit, $eligible_revision_limit, $current_revision_limit, $extra_text = 'Days');
				$html .= $this->get_dropdown_option_html(WPTC_DEFAULT_MAX_REVISION_LIMIT, WPTC_DEFAULT_MAX_REVISION_LIMIT, $current_revision_limit, $extra_text = 'Days');
				$html .= $this->get_dropdown_option_html(WPTC_FALLBACK_REVISION_LIMIT_DAYS, WPTC_FALLBACK_REVISION_LIMIT_DAYS, $current_revision_limit, $extra_text = 'Days');
				break;
			case ($eligible_revision_limit == WPTC_DEFAULT_MAX_REVISION_LIMIT):
				$html .= $this->get_dropdown_option_html(WPTC_DEFAULT_MAX_REVISION_LIMIT, WPTC_DEFAULT_MAX_REVISION_LIMIT, $current_revision_limit, $extra_text = 'Days');
				$html .= $this->get_dropdown_option_html(WPTC_FALLBACK_REVISION_LIMIT_DAYS, WPTC_FALLBACK_REVISION_LIMIT_DAYS, $current_revision_limit, $extra_text = 'Days');
	...
	public function get_schedule_times_div_wptc() {

		$div = '';
		$already_selected_schedule = $this->config->get_option('schedule_time_str');

		for ($i = 1; $i <= 24; $i++) {
			$time = date("g:i a", strtotime("$i:00"));
			$div .= $this->get_dropdown_option_html($time, $time, $already_selected_schedule);
		}
	...
	public function admin_footer_text( $admin_footer_text ) {
		...
		if ( isset( $_REQUEST[ 'page' ] ) && strpos($_REQUEST[ 'page' ], 'wp-time-capsule' ) !== false ) {
			$admin_footer_text =  __( '<span>If you have any questions, see the <a href="http://wptc.helpscoutdocs.com/article/7-commonly-asked-questions" target="_blank">FAQs</a> or email us at <a href="mailto:help@wptimecapsule.com?Subject=Contact" target="_top">help@wptimecapsule.com</a></span><br>', 'wp-time-capsule' );
	...
	public function save_settings_revision_limit($requested_revision_limit){
		...
		if($requested_revision_limit <= WPTC_DEFAULT_MAX_REVISION_LIMIT){
			$this->config->set_option('settings_revision_limit', $requested_revision_limit);
			return ;
		}
		...
		switch($default_repo){
			case 'g_drive':
				...
				break;
			case 'dropbox':
				$modified_revision_limits = $cloud_repo->validate_max_revision_limit();
			...
			case 's3':
				$modified_revision_limits = $cloud_repo->validate_max_revision_limit();
				if ($modified_revision_limits < $requested_revision_limit) {
					return array('title' => 'Oops...', 'message' => 'Add life cycle to enable revisions more than ' . WPTC_DEFAULT_MAX_REVISION_LIMIT . ' days ' . $help_doc, 'type' => 'error');
				}
				break;
	...
	public function get_user_excluded_files_more_than_size(){
		return WPTC_Base_Factory::get('Wptc_ExcludeOption')->get_user_excluded_files_more_than_size();
	}
}