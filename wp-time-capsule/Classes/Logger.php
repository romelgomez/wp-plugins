...
class WPTC_Logger {
	...
	public function set_log_now($type, $msg, $action_id, $data, $show_user) {
		...
		$DBLogArray = array('activated_plugin', 'deactivated_plugin', 'remove_currentacc', 'backup_start', 'backup_progress', 'backup_error', 'backup_complete', 'restore_start', 'restore_complete', 'restore_error', 'connection_error', 'staging_progress' , 'staging_failed', 'staging_completed', 'staging_start');
		if ($action_id == "") {
			$action_id = time();
		}
		if ($type != "") {
