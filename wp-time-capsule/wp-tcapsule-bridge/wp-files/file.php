...
function _unzip_file_ziparchive($file, $to, $needed_dirs = array()) {
	...
	if (!$wp_filesystem) {
		...
		if (empty($wp_filesystem)) {
			send_response_wptc('FS_INIT_FAILED-021');
			return false;
		}
	}
	$z = new ZipArchive();
...
function _unzip_file_pclzip($file, $to, $needed_dirs = array()) {
	...
	if (!$wp_filesystem) {
		initiate_filesystem_wptc();
		if (empty($wp_filesystem)) {
			send_response_wptc('FS_INIT_FAILED-022');
			return false;
		}
	}
	mbstring_binary_safe_encoding();

	require_once ABSPATH . BRIDGE_NAME_WPTC . '/wp-files/class-pclzip.php'; //modified
...
function copy_dir($from, $to, $skip_list = array()) {
	...
	if (!$wp_filesystem) {
		initiate_filesystem_wptc();
		if (empty($wp_filesystem)) {
			send_response_wptc('FS_INIT_FAILED-023');
			return false;
		}
	}
...
function WP_Filesystem($args = false, $context = false, $allow_relaxed_file_ownership = false) {
	...
	//require_once ABSPATH . BRIDGE_NAME_WPTC . '/wp-files/class-wp-filesystem-base.php'; //modified
	...
	if (!class_exists("WP_Filesystem_$method")) {
		 ...
		 */
		$abstraction_file = apply_filters('filesystem_method_file', ABSPATH . BRIDGE_NAME_WPTC . '/wp-files/class-wp-filesystem-' . $method . '.php', $method); //modified
