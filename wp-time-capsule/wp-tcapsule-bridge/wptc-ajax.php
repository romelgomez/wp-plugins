...
class WPTC_Restore_Download {
	...
	public function __construct($type = false) {
		...
		//start sentry logs
		// $this->common_include_files->start_sentry_restore_logs();
	...
	private function check_for_other_operations(){
		...
		if (isset($this->post_data['action']) && $this->post_data['action'] == 'get_last_php_error' ) {
	...
	public function set_flags_and_add_database_file($files_to_restore) {
		...
		//This never be empty because we add some dummy date on 162th line even its empty
		if (empty($files_to_restore)) {
			return true;
		}
		...
		WPTC_Factory::get('logger')->log(sprintf(__('Restore started on %s.', 'wptc'), date("l F j, Y", strtotime(current_time('mysql')))), 'restore_start', $this->restore_id);
		...
		), 'restore_start', $this->restore_id);

		if (ini_get('safe_mode')) {
			WPTC_Factory::get('logger')->log(__("Safe mode is enabled on your server so the PHP time and memory limit cannot be set by the Restore process. So if your Restore fails it's highly probable that these settings are too low.", 'wptc'), 'restore_error', $this->restore_id);
		}

		$cur_res_b_id = $this->get_cur_res_b_id();

		$processed_base = new WPTC_Processed_Files();
		...
		if (empty($cur_res_b_id)) {
			...
			$backup_type = $processed_base->backup_type_check($selected_restore_id);
			...
			WPTC_Factory::get('logger')->log(__("Files prepared for Restoring.", 'wptc'), 'restore_error', $this->restore_id);
			...
			$backup_type = $processed_base->backup_type_check($cur_res_b_id);
			...
			$this->processed_files->add_files_for_restoring($files_to_restore);
	...
	public function start_restore() {
		...
		try {
			if (!$this->cloud_repo || !$this->cloud_repo->is_authorized($is_restore = true)) {
				$logger->log(__('Your '.DEFAULT_REPO.' account is not authorized yet.', 'wptc'), 'restore_error', $this->restore_id);
			...
			$logger->log(__('Restore complete.', 'wptc'), 'restore_complete', $this->restore_id);
			$logger->log(sprintf(__('A total of %s files were processed.', 'wptc'), $this->processed_file_count), 'restore_complete', $this->restore_id);
			$logger->log(sprintf(
				__('A total of %dMB of memory was used to complete this restore.', 'wptc'),
				(memory_get_usage(true) / 1048576)
			), 'restore_complete', $this->restore_id);
			...
			if ($e->getMessage() == 'Unauthorized') {
				$logger->log(__('The plugin is no longer authorized with Cloud Repo.', 'wptc'), 'restore_error', $this->restore_id);
			} else {
				$logger->log(__('A fatal error occured: ', 'wptc') . $e->getMessage(), 'restore_error', $this->restore_id);
			}
	...
	public function add_files_and_start_download($file = null, $version = null, $site_abspath = null) {
		...
		$site_abspath = get_tcsanitized_home_path();
		...
		if (!$got_selected_files_to_restore) {
			...
			WPTC_Factory::get('logger')->log(__("Filelist prepared for Restoring Selected Files.", 'wptc'), 'restore_process', $this->restore_id);
		}
		...
		if (!$got_restore_file_list) {
			...
			WPTC_Factory::get('logger')->log(__("Starting to prepare the Filelist for Restoring.", 'wptc'), 'restore_process', $this->restore_id);
			...
			if (!empty($cur_res_b_id)) {
				...
				WPTC_Factory::get('logger')->log(__("Filelist prepared for Restoring.", 'wptc'), 'restore_process', $this->restore_id);
		...
		while ($is_queue) {
			foreach ($restore_queue as $key => $value) {
				...
				$file = wptc_add_abspath($relative_file, false);
				...
				if (is_array($cloud_response) && isset($cloud_response['error']) || empty($cloud_response)) {
					...
					WPTC_Factory::get('logger')->log(__("Limit reached during download : .", 'wptc'), 'restore_process', $this->restore_id);
	...
	public function process_add_selected_files_to_restore() {
		...
		foreach ($files_to_restore_tmp as $files_or_folders => $v) {
			if ($files_or_folders == 'files') {
				...
				$this->processed_files->add_files_for_restoring($v, $check_hash = true);
	...
	private function get_files_by_folders(){
		...
		while(!$break){
			...
			$path = wptc_add_abspath($relative_path, false);
	...
	private function is_skip($file){
		...
		if (stripos($file, 'wp-time-capsule') !== false || (stripos($file, 'tCapsule') !== false && stripos($file, 'wptc_current_files_state') === false) || strpos($file, 'error_log') !== false) {
			return true;
		}
		...
		if (false !== stripos($file, '/tCapsule/backups/') && false === stripos($file, 'wptc_current_files_state') ) {
			return true;
		}
	...
	public function add_future_files_and_get_restorable_files($file, &$offset) {
		...
		$future_file['file'] = wptc_remove_abspath($file);
	...
	public function get_recorded_files_of_this_folder_to_restore_table($folder_name, $folder_res_id) {
		...
		if($this->wpdb->use_mysqli){
			while ( $recent_revision = @mysqli_fetch_object($result)){
				$this->select_downloaded_needed_files($recent_revision, $total_counter, $offset, $current_loop_counter, $folder_res_id);
			}
		} else {
		...
		if (!empty($this->prepared_file_array)) {
			...
			$this->processed_files->add_files_for_restoring($this->prepared_file_array);
		}
	...
	private function delete_dir_if_empty($fs_safe_dir){
		...
		if ( strpos($fs_safe_dir, ABSPATH) === false || $fs_safe_dir === ABSPATH || $fs_safe_dir === WP_CONTENT_DIR) {
	...
	public function get_recorded_files_to_restore_table() {
		...
		$result = $this->processed_files->get_sql_files_iterator_for_site($cur_res_b_id, $offset);
		...
		if($this->wpdb->use_mysqli){
			while ( $recent_revision = @mysqli_fetch_object($result)){
				$this->select_downloaded_needed_files($recent_revision, $total_counter, $offset, $current_loop_counter, $cur_res_b_id);
			}
		} else {
			while ( $recent_revision = @mysql_fetch_object($result)){
				$this->select_downloaded_needed_files($recent_revision, $total_counter, $offset, $current_loop_counter, $cur_res_b_id);
	...
	private function select_downloaded_needed_files($recent_revision, &$total_counter, $offset, &$current_loop_counter, $cur_res_b_id){
			wptc_manual_debug('', 'during_get_restorable_files', 500);
			...
			$path = wptc_add_abspath($recent_revision->file, false);
			...
			if ($is_same_hash) {
				// wptc_log($recent_revision->file, '-----Hash or size does not modified so continue-----------');
			...
			$this->prepared_file_array[$recent_revision->revision_id]['backup_id'] 				= $cur_res_b_id;


			// /Check after certain limit

			if($current_loop_counter < WPTC_RESTORE_ADDING_FILES_LIMIT ){
				$this->is_checking_restorable_files_timedout($total_counter);
				return;
			}
			...
			//resetting flags for next iterator
			$current_loop_counter = 0;
