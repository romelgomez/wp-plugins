...
class WC_Aelia_Currencies_Manager {
	...
	public function get_currency_by_host($host, $default_currency) {
		...
		Logger::instance()->debug(__('Attempting to select currency by geolocation.', Definitions::TEXT_DOMAIN),
															array(
																"Host" => $host,
		...
		if($country_code === false) {
			Logger::instance()->info(__('Geolocation failed, selecting default currency.', Definitions::TEXT_DOMAIN),
															 array(
																	'Host' => $host,
