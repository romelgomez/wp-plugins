...
class PluginOrganizer {
	...
	function check_version() {
		...
		if (get_option("PO_version_num") != "9.3.4" && !in_array($pagenow, array("plugins.php", "update-core.php", "update.php"))) {
			$this->activate();
		}
	}
	...
	function activate() {
		...
		if (get_option("PO_version_num") != "9.3.4") {
			update_option("PO_version_num", "9.3.4");
		}
	...
	function change_plugin_filter_title($title) {
		...
		if (is_object($post) && get_post_type($post->ID) == 'plugin_filter') {
			if (isset($this->postedData['PO_filter_name']) && $this->postedData['PO_filter_name'] != '') {
				return $this->postedData['PO_filter_name'];
				...
				return $this->postedData['PO_permalink_filter'];
			}
		} else {
			return $title;
		}
	...
	function save_post_meta_box($post_id) {
		...
		if ( (defined('DOING_AUTOSAVE') && DOING_AUTOSAVE) || wp_is_post_revision($post_id) || !current_user_can( 'edit_post', $post_id ) || ($ptSettingsFound !=1 && !current_user_can( 'activate_plugins' )) || !in_array(get_post_type($post_id), $supportedPostTypes) || !isset($this->postedData['poSubmitPostMetaBox'])) {
			return $post_id;
		}
		...
		if (get_post_type($post_id) != 'plugin_filter') {
			...
			if (isset($this->postedData['PO_pl_id'])) {
				foreach($this->postedData['PO_pl_id'] as $plID) {
					...
					foreach($availableRoles as $roleID=>$roleName) {
						if (!is_numeric($plID)) {
							$permalinks[] = array($plID, $this->fix_trailng_slash($this->postedData['PO_permalink_filter_'.$plID]), $roleID);
						} else {
							...
							if (!is_numeric($pluginListID)) {
								$permalinks[] = array('tmp_tmp', $this->fix_trailng_slash($this->postedData['PO_permalink_filter_'.$plID]), $roleID);
							} else {
								$permalinks[] = array($pluginListID, $this->fix_trailng_slash($this->postedData['PO_permalink_filter_'.$plID]), $roleID);
							}
						}
					}
				}
		...
		if (isset($this->postedData['PO_post_priority']) && is_numeric($this->postedData['PO_post_priority'])) {
			$postPriority = $this->postedData['PO_post_priority'];
		} else {
			$postPriority = 0;
		}
	...
	function get_submitted_plugin_lists($role) {
		...
		$checkPluginList = (isset($this->postedData['PO_disabled_std_plugin_list'][$role])) ? $this->postedData['PO_disabled_std_plugin_list'][$role] : '';
		...
		if (get_option('PO_disable_plugins_mobile') == 1) {
			...
			$checkPluginList = (isset($this->postedData['PO_disabled_mobile_plugin_list'][$role])) ? $this->postedData['PO_disabled_mobile_plugin_list'][$role] : '';
		...
		$checkPluginList = (isset($this->postedData['PO_disabled_std_group_list'][$role])) ? $this->postedData['PO_disabled_std_group_list'][$role] : '';
		...
		if (get_option('PO_disable_plugins_mobile') == 1) {
			...
			$checkPluginList = (isset($this->postedData['PO_disabled_mobile_group_list'][$role])) ? $this->postedData['PO_disabled_mobile_group_list'][$role] : '';
